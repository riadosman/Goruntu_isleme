...
while cap.isOpened():
  ...
  if results.face_landmarks:
        ...
        if avg_ear > 0.3:
          cv2.putText(frame, "Goz Aciktir", (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
          goz_kapali_baslangic = None
        else:
          if goz_kapali_baslangic is None:
              goz_kapali_baslangic = time.time() 
          print(f"ilk kapanma ani {goz_kapali_baslangic}")
          gecen_sure = time.time() - goz_kapali_baslangic
          cv2.putText(frame, f"EAR: {avg_ear:.2f} | Durum: GOZ KAPALI | Sure: {gecen_sure:.2f} sn", (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
          if gecen_sure > 2:
              cv2.putText(frame, "UYARI: GOZ 2+ SN KAPALI!", (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
